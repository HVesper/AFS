...
class AFSTrainer:
    def __init__(self, config):
        if getattr(config, "trainer", None) != "afs_training":
            raise ValueError("This trainer only supports trainer=afs_training")
        if getattr(config, "distribution_loss", None) != "afs_training":
            raise ValueError("This trainer only supports distribution_loss=afs_training")
        ...
        self.model = AFSModel(config, device=self.device)
        ...
        if self.is_main_process:
            print(
                f"AFS trainer ready | step={self.step} | lora={self.is_lora_enabled} "
    ...
    def _load_base_generator_checkpoint(self):
        ...
        if not ckpt_path:
            raise ValueError("generator_ckpt is required for afs_training training unless model_kwargs.debug=true")
    ...
    def start_new_sequence(self, window_start_override=None):
        ...
        gt_latents = batch.get("gt_chunk_latents", batch.get("gt_latents", None))
        if gt_latents is None:
            raise ValueError("AFS streaming requires `gt_chunk_latents` or `gt_latents` in each batch.")
        if gt_latents.ndim == 6:
            gt_latents = gt_latents.permute(0, 1, 3, 2, 4, 5).flatten(1, 2)

        batch_size = gt_latents.shape[0]
        if "global_text_embeddings" in batch or "prompt_embeds" in batch:
            prompt_embeds = batch.get("global_text_embeddings", batch.get("prompt_embeds"))
            prompt_embeds = prompt_embeds.to(device=self.device, dtype=self.dtype)
            conditional_dict = {"prompt_embeds": prompt_embeds}
        else:
            if self.model.text_encoder is None:
                raise KeyError("global text embeddings are missing and use_text_encoder=false")
            if text_prompts is None:
                raise KeyError("`prompts` is required when embeddings are not precomputed")
            with torch.no_grad():
        ...
        training_batch = AFSTrainingBatch(
            ...
            global_text_masks=batch.get("global_text_masks", batch.get("global_text_mask")),
            chunk_text_embeddings=batch["chunk_text_embeddings"],
            chunk_text_masks=batch.get("chunk_text_masks"),
            gt_chunk_latents=batch["gt_chunk_latents"],
            valid_chunk_mask=batch.get("valid_chunk_mask"),
        )
        # TODO(data): populate chunk_text_embeddings/masks and valid_chunk_mask in
        # the offline data pipeline. No captioner or Video-VLM runs in Trainer.

        self.streaming_model.setup_afs_sequence(
    ...
    def training_step(self, batch: AFSTrainingBatch) -> dict[str, torch.Tensor]:
        """Run the real upstream AFS forward/backward path for a prepared batch."""
        batch.validate()
        batch = batch.to(self.device, self.dtype)
        conditional_dict = {"prompt_embeds": batch.global_text_embeddings}
        unconditional_dict = self._get_unconditional_dict(
            len(batch.sample_ids), reference_prompt_embeds=batch.global_text_embeddings
        )
        self.streaming_model.setup_afs_sequence(
            conditional_dict=conditional_dict,
            unconditional_dict=unconditional_dict,
            gt_latents=batch.frame_major_latents(),
    ...
    def rollout_student_chunk(self, batch, chunk_index, student_cache):
        raise RuntimeError("Chunk rollout is executed by AFSStreamingTrainingModel.compute_afs_generator_loss")

    @torch.no_grad()
    def evaluate_teacher_on_student_states(self, batch, rollout, teacher_cache):
        raise RuntimeError("Teacher evaluation is fused into the upstream streaming loop to preserve Wan KV semantics")

    def compute_afs_loss(self, student_velocities, teacher_velocities):
        from afs.loss import AFSLoss
        return AFSLoss()(student_velocities, teacher_velocities)
    ...
    def fwdbwd_one_step_afs(self, post_backward_callback=None):
        self.model.eval()
        if (
            ...
            if self.is_main_process:
                print(f"EMA created before AFS teacher forward at step {self.step}")
        if self.generator_ema is not None:
            self.streaming_model.set_afs_teacher_ema_shadow(self.generator_ema.shadow)
        else:
            self.streaming_model.set_afs_teacher_ema_shadow(None)
        if bool(getattr(self.config, "afs_debug_teacher_swap", False)) and self.is_main_process:
        ...
        backward_per_chunk = bool(getattr(self.config, "afs_backward_per_chunk", False))
        generator_loss, generator_log_dict = self.streaming_model.compute_afs_generator_loss(
    ...
    def _maybe_save_afs_rollout_debug(self):
        save_iters = int(getattr(self.config, "afs_debug_save_rollout_iters", 0) or 0)
        if save_iters <= 0:
            return
        ...
        if self.is_main_process:
            output_root = getattr(self.config, "afs_debug_rollout_dir", None)
            if output_root is None:
                output_root = str(Path(self.output_path) / "afs_rollout_debug")
            output_root = Path(output_root)
            output_root.mkdir(parents=True, exist_ok=True)

            fps = int(getattr(self.config, "afs_debug_rollout_fps", 16))
            decode_chunk_size = int(getattr(self.config, "afs_debug_rollout_decode_chunk_size", 120))
            student_path = output_root / f"step_{self.step:06d}_student.mp4"
            teacher_path = output_root / f"step_{self.step:06d}_teacher.mp4"

            with torch.no_grad():
                self.streaming_model.debug_afs_rollout_videos(
                    student_video_path=str(student_path),
                    teacher_video_path=str(teacher_path),
                    fps=fps,
                    decode_chunk_size=decode_chunk_size,
                )
            print(f"Saved AFS rollout debug videos: {student_path}, {teacher_path}")
    ...
    def train(self):
        ...
        while self.step < max_iters:
            ...
            for _ in range(self.gradient_accumulation_steps):
                extra_gen = self.fwdbwd_one_step_afs(
            ...
            if self.use_lora_teacher_adapter:
                ...
                self.update_ema_teacher()
            self.step += 1
            self._maybe_save_afs_rollout_debug()
...
# Compatibility for the upstream import path.
Trainer = AFSTrainer
