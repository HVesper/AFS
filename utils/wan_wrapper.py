...
def _resolve_model_root(model_root: Optional[str]) -> str:
    root = os.environ.get("WAN_MODEL_ROOT", model_root or DEFAULT_WAN_MODEL_ROOT)
...
class WanTextEncoder(torch.nn.Module):
    def __init__(self, model_root: Optional[str] = None) -> None:
        ...
        self.tokenizer = HuggingfaceTokenizer(
            name=os.path.join(model_root, "google", "umt5-xxl"),
            seq_len=512,
            clean='whitespace',
        )
...
class WanVAEWrapper(torch.nn.Module):
    def __init__(self, model_root: Optional[str] = None):
        ...
        self.model = _video_vae(
            pretrained_path=os.path.join(model_root, "Wan2.1_VAE.pth"),
            z_dim=16,
        ).eval().requires_grad_(False)
...
class WanDiffusionWrapper(torch.nn.Module):
    def __init__(
        ...
        if is_causal:
            if debug:
                self.model = CausalWanModel(num_layers=1, local_attn_size=local_attn_size, sink_size=sink_size)
            else:
