...
class AFSStreamingTrainingPipeline:
    def __init__(self,
        ...
        # When enabled, always run all denoising steps and exit only at the last step.
        self.force_full_denoise = False

        self.local_attn_size = kwargs.get("local_attn_size", -1)
        self.num_transformer_blocks = self._get_model_attr("num_layers", self.num_transformer_blocks)
        self.num_attention_heads = self._get_model_attr("num_heads", 12)
        hidden_dim = self._get_model_attr("dim", self.num_attention_heads * 128)
        self.attention_head_dim = hidden_dim // self.num_attention_heads
    ...
    def _get_model_attr(self, name: str, default):
        ...
        *,
        ...
        for block_index, current_num_frames in enumerate(all_num_frames):
            ...
            for step_idx, current_timestep in enumerate(self.denoising_step_list):
                if self.force_full_denoise:
                    exit_flag = step_idx == (len(self.denoising_step_list) - 1)
                else:
                    exit_flag = (
    ...
    def _initialize_crossattn_cache(self, batch_size, dtype, device):
        ...
        for _ in range(self.num_transformer_blocks):
            crossattn_cache.append({
                "k": torch.zeros([batch_size, 512, self.num_attention_heads, self.attention_head_dim], dtype=dtype, device=device),
                "v": torch.zeros([batch_size, 512, self.num_attention_heads, self.attention_head_dim], dtype=dtype, device=device),
                "is_init": False
            })
