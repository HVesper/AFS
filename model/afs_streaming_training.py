...
from pipeline.afs_streaming_training import AFSStreamingTrainingPipeline
from afs.text_condition import AFSTeacherConditionSelector
from afs.types import AFSDenoisingState, AFSStudentChunkRollout, AFSTrainingBatch


class AFSStreamingTrainingModel:
    """
    A model wrapper for AFS streaming training.

    This class owns the AFS rollout state, student/teacher KV caches, teacher
    ...
    def __init__(self, base_model, config):
        ...
        self.afs_training = getattr(config, "distribution_loss", "") == "afs_training"
        self.afs_loss_type = getattr(config, "afs_loss_type", "flow")
        self.afs_loss_frames = int(getattr(config, "afs_loss_frames", 42))
        self.afs_max_loss_frames = int(getattr(config, "afs_max_loss_frames", 0))
        self.afs_loss_start_frame = max(0, int(getattr(config, "afs_loss_start_frame", 21)))
        self.afs_use_gt_first_chunk = bool(getattr(config, "afs_use_gt_first_chunk", True))
        self.afs_teacher_context_mode = getattr(config, "afs_teacher_context_mode", "gt_kv")
        self.afs_student_context_mode = getattr(config, "afs_student_context_mode", "generated_kv")
        self.afs_uniform_timestep = bool(getattr(config, "afs_uniform_timestep", True))
        self.afs_loss_step_mode = getattr(config, "afs_loss_step_mode", "single")
        self.afs_loss_flag = int(getattr(config, "afs_loss_flag", -1))
        self.afs_shared_flag_per_window = bool(getattr(config, "afs_shared_flag_per_window", True))
        self.afs_use_relative_sink = bool(getattr(config, "afs_use_relative_sink", False))
        self.afs_teacher_use_future_gt_context = bool(
            getattr(config, "afs_teacher_use_future_gt_context", False)
        )
        self.afs_teacher_future_gt_num_chunks = max(
            0,
            int(getattr(config, "afs_teacher_future_gt_num_chunks", 1)),
        )
        self.afs_teacher_trajectory_mode = str(
            getattr(config, "afs_teacher_trajectory_mode", "student")
        ).lower()
        if self.afs_teacher_trajectory_mode not in {"student", "teacher"}:
            raise ValueError("afs_teacher_trajectory_mode must be 'student' or 'teacher'")
        self.teacher_text_condition_mode = str(
            getattr(config, "teacher_text_condition_mode", "global")
        ).lower()
        self.teacher_condition_selector = AFSTeacherConditionSelector()
        self.shared_prefix_chunks = int(getattr(config, "shared_prefix_chunks", 1))
        self.use_gt_assisted_teacher_cache = bool(
            getattr(config, "use_gt_assisted_teacher_cache", True)
        )
        self.detach_rollout_transition = bool(getattr(config, "detach_rollout_transition", True))
        if self.shared_prefix_chunks not in {0, 1}:
            raise ValueError("AFS supports shared_prefix_chunks=0 or 1")
        if not self.detach_rollout_transition:
            raise ValueError("detach_rollout_transition=False is not supported by AFS streaming")
        ...
        if self.inference_pipeline is None:
            ...
            self.inference_pipeline = base_model.inference_pipeline
        if not isinstance(self.inference_pipeline, AFSStreamingTrainingPipeline):
            self.inference_pipeline = self._build_afs_inference_pipeline(base_model)
    ...
    def _build_afs_inference_pipeline(self, base_model):
        ...
        return AFSStreamingTrainingPipeline(
    ...
    @staticmethod
    def _reset_crossattn_cache(crossattn_cache: list):
        """Invalidate cached text K/V when the teacher moves to another chunk."""
        for cache in crossattn_cache:
            cache["k"].zero_()
            cache["v"].zero_()
            cache["is_init"] = False

    def _teacher_condition(self, chunk_index: int) -> dict:
        batch = self.state["afs_training_batch"]
        return self.teacher_condition_selector.select(
            batch, chunk_index, self.teacher_text_condition_mode
        ).as_model_dict()
    ...
    def set_afs_teacher_ema_shadow(self, ema_shadow: Optional[dict]):
        self.afs_teacher_ema_shadow = ema_shadow
    ...
    def _swap_generator_to_ema(self):
        debug_swap = bool(getattr(self.config, "afs_debug_teacher_swap", False))
        debug_rank0 = not dist.is_initialized() or dist.get_rank() == 0
        debug_prints = int(getattr(self, "_afs_debug_teacher_swap_prints", 0))
        debug_max_prints = int(getattr(self.config, "afs_debug_teacher_swap_max_prints", 20))
        ...
        if lora_model is not None and "teacher" in getattr(lora_model, "peft_config", {}):
            ...
            if should_print_debug:
                ...
                self._afs_debug_teacher_swap_prints = debug_prints + 1
            return ("adapter", active_adapter or "student")
        if not self.afs_teacher_ema_shadow:
            if should_print_debug:
                print("[TEACHER SWAP DEBUG] mode=student_fallback reason=no_ema_shadow")
                self._afs_debug_teacher_swap_prints = debug_prints + 1
            if bool(getattr(self.config, "afs_require_teacher_ema", False)):
                raise RuntimeError(
                    "AFS teacher EMA shadow is missing, so teacher would fall back to student. "
        ...
        with FSDP.summon_full_params(self.generator, writeback=True, rank0_only=False):
            for raw_name, p in self.generator.named_parameters():
                ...
                ema_v = self.afs_teacher_ema_shadow.get(n, None)
        ...
        if should_print_debug:
            print(
                ...
                f"shadow_keys={len(self.afs_teacher_ema_shadow)} "
    ...
    def _finalize_relative_sink_capture(self, kv_cache_list: list):
        ...
        if missing_cache_indices:
            raise RuntimeError(
                "Failed to capture AFS relative sink. "
    ...
    def setup_afs_sequence(
        self,
        conditional_dict: Dict,
        unconditional_dict: Dict,
        gt_latents: torch.Tensor,
        window_start_override: Optional[int] = None,
        training_batch: Optional[AFSTrainingBatch] = None,
    ):
        self.reset_state()
        ...
        if training_batch is None:
            raise ValueError("AFS requires an AFSTrainingBatch from Stage 1 outputs")
        training_batch = training_batch.to(self.device, self.dtype)
        training_batch.validate()
        self.state["afs_training_batch"] = training_batch
        if gt_latents is None:
            raise ValueError("AFS streaming requires gt_latents in batch.")
        ...
        max_loss_frames = gt_total_frames if self.afs_max_loss_frames <= 0 else min(gt_total_frames, self.afs_max_loss_frames)
        max_loss_frames = max(block, (max_loss_frames // block) * block)
        desired_loss_frames = min(int(self.afs_loss_frames), max_loss_frames)
        desired_loss_frames = max(block, (desired_loss_frames // block) * block)
        max_start = gt_total_frames - desired_loss_frames
        if max_start < 0:
            raise ValueError(f"Invalid AFS window: gt_total_frames={gt_total_frames}, desired_loss_frames={desired_loss_frames}")
        ...
        if self.afs_use_relative_sink:
            self._prepare_relative_sink_mode(self.student_kv_cache1)
            self._prepare_relative_sink_mode(self.teacher_kv_cache1)

        self.state["afs_gt_latents"] = gt_latents
        self.state["afs_window_start"] = int(window_start)
        self.state["afs_window_frames"] = int(desired_loss_frames)
        self.state["afs_student_chunks"] = []
        self.state["afs_teacher_chunks"] = []
        self.state["afs_generated_chunk_spans"] = []
        self.state["current_length"] = 0
        use_shared_prefix = self.shared_prefix_chunks == 1
        if self.afs_use_relative_sink and not use_shared_prefix:
            raise ValueError("afs_use_relative_sink requires shared_prefix_chunks=1")
        ...
        if use_shared_prefix:
            ...
            self._write_chunk_to_cache(
                chunk_latents=first_chunk,
                conditional_dict=self._teacher_condition(window_start // block),
                timestep=timestep0,
            ...
            self.state["afs_student_chunks"].append(first_chunk.detach())
            self.state["afs_teacher_chunks"].append(first_chunk.detach())
    ...
    def compute_afs_generator_loss(
        self,
        backward_per_chunk: bool = False,
        loss_scale: float = 1.0,
        post_backward_callback: Optional[Callable[[], None]] = None,
    ) -> Tuple[torch.Tensor, Dict[str, Any]]:
        if self.state.get("afs_gt_latents", None) is None:
            raise ValueError("AFS sequence is not initialized. Please call setup_afs_sequence first.")

        gt_latents = self.state["afs_gt_latents"]
        window_start = int(self.state["afs_window_start"])
        window_frames = int(self.state["afs_window_frames"])
        ...
        if num_denoising_steps <= 0:
            raise ValueError("Empty denoising_step_list for AFS training.")

        def _sample_flag_idx():
            if self.afs_loss_flag >= 0:
                flag_idx = min(self.afs_loss_flag, num_denoising_steps - 1)
        ...
        def _loss_step_indices(flag_idx):
            if self.afs_loss_step_mode == "all":
                # Z-Image D-AFS style: supervise every denoising step, while
                # transition dynamics below remain stop-gradient.
                return set(range(num_denoising_steps))
            if self.afs_loss_step_mode == "prefix":
            ...
            if self.afs_loss_step_mode == "single":
                # Self-forcing style: only selected step contributes gradient/loss.
                return {flag_idx}
            raise ValueError(
                f"Unsupported afs_loss_step_mode={self.afs_loss_step_mode}. "
        ...
        shared_flag_idx = (
            None
            if self.afs_loss_step_mode == "all"
            else (_sample_flag_idx() if self.afs_shared_flag_per_window else None)
        )
        for chunk_idx in range(total_chunks):
            flag_idx = -1 if self.afs_loss_step_mode == "all" else (
            ...
            enable_chunk_loss = scheduled_start >= self.afs_loss_start_frame
        ...
        total_loss = None
        self.state["afs_student_rollouts"] = []
        ...
        while current_length < window_frames:
            ...
            absolute_chunk_index = abs_chunk_start // block
            teacher_conditional_dict = self._teacher_condition(absolute_chunk_index)
            valid_chunk_mask = self.state["afs_training_batch"].valid_chunk_mask
            chunk_valid = None
            if valid_chunk_mask is not None:
                if absolute_chunk_index >= valid_chunk_mask.shape[1]:
                    raise IndexError(f"Missing valid_chunk_mask entry for chunk {absolute_chunk_index}")
                chunk_valid = valid_chunk_mask[:, absolute_chunk_index].to(torch.bool)
            # The visual cache persists; text K/V is scoped to exactly one target chunk.
            self._reset_crossattn_cache(self.teacher_crossattn_cache)
            ...
            student_chunk = None
            teacher_chunk = None
            chunk_rollout = AFSStudentChunkRollout(chunk_index=absolute_chunk_index)
            ...
            if self.afs_teacher_use_future_gt_context and self.afs_teacher_future_gt_num_chunks > 0:
                ...
                capture_future_chunks = min(
                    self.afs_teacher_future_gt_num_chunks,
                ...
                if capture_future_chunks > 0:
                    ...
                    captured_future = self._capture_future_gt_context_for_cache(
                        ...
                        conditional_dict=teacher_conditional_dict,
            ...
            for step_idx, current_timestep in enumerate(denoising_steps):
                ...
                chunk_rollout.denoising_states.append(AFSDenoisingState(
                    chunk_index=absolute_chunk_index,
                    step_index=step_idx,
                    noisy_latent=noisy_input.detach(),
                    timestep=timestep.detach(),
                    student_velocity=student_flow,
                ))

                if need_step_loss:
                    per_sample_dims = tuple(range(1, student_flow.ndim))
                    if self.afs_loss_type == "flow":
                        per_sample_loss = ((student_flow - teacher_flow.detach()) ** 2).mean(per_sample_dims)
                    elif self.afs_loss_type == "x0":
                        per_sample_dims = tuple(range(1, student_chunk.ndim))
                        per_sample_loss = ((student_chunk - teacher_chunk.detach()) ** 2).mean(per_sample_dims)
                    else:
                        raise ValueError(
                            f"Unsupported afs_loss_type={self.afs_loss_type}. Use 'flow' or 'x0'."
                        )
                    if chunk_valid is None:
                        step_loss = per_sample_loss.mean()
                    else:
                        weights = chunk_valid.to(per_sample_loss.dtype)
                        step_loss = (per_sample_loss * weights).sum() / weights.sum().clamp_min(1.0)
                # if (
                ...
                if step_idx < num_denoising_steps - 1:
                    ...
                    with torch.no_grad():
                        ...
                        if self.afs_teacher_trajectory_mode == "teacher":
            ...
            if student_chunk is None or teacher_chunk is None:
                raise RuntimeError("AFS chunk rollout produced no outputs.")
            chunk_rollout.clean_chunk = student_chunk.detach()
            self.state["afs_student_rollouts"].append(chunk_rollout)
            ...
            with torch.no_grad():
                ...
                # AR-consistent teacher history keeps the latest Student chunk;
                # older generated spans are replaced by GT below.
            ...
            generated_spans = self.state["afs_generated_chunk_spans"]
            generated_spans.append((int(abs_chunk_start), int(current_num_frames)))

            if self.use_gt_assisted_teacher_cache and self.afs_teacher_context_mode == "gt_kv" and len(generated_spans) >= 2:
                ...
                if gt_latents.shape[1] >= replace_end:
                    ...
                    self._replace_cache_chunk_with_gt(
                        ...
                        conditional_dict=self._teacher_condition(replace_start // block),
                        chunk_start_frame=replace_start,
                        use_teacher_ema=True,
                    )

            self.state["afs_student_chunks"].append(student_chunk.detach())
            self.state["afs_teacher_chunks"].append(teacher_chunk.detach())
            current_length += current_num_frames

        self.state["current_length"] = current_length
        if self.afs_loss_type not in {"flow", "x0"}:
            raise ValueError(f"Unsupported afs_loss_type={self.afs_loss_type}. Use 'flow' or 'x0'.")
        if total_loss is None or total_loss_terms == 0:
            raise RuntimeError("No AFS loss terms were accumulated. Check afs_loss_step_mode/flag settings.")
        ...
        log_dict = {
            ...
            "afs_window_start": torch.tensor(window_start, device=self.device),
            "afs_window_frames": torch.tensor(window_frames, device=self.device),
            "afs_loss_frames": torch.tensor(total_supervised_frames, device=self.device),
            "afs_loss_start_frame": torch.tensor(self.afs_loss_start_frame, device=self.device),
            "afs_loss_terms": torch.tensor(total_loss_terms, device=self.device),
            "afs_avg_flag": torch.tensor(avg_flag, device=self.device),
        }
        return loss, log_dict

    @torch.no_grad()
    def debug_afs_rollout_videos(
        self,
        ...
        """
        Run the AFS training rollout without loss/backward and save student/teacher videos.

        This mirrors the rollout path in `compute_afs_generator_loss`: it uses the
        ...
        in the teacher cache. Call `setup_afs_sequence(...)` before this method.
        """
        if self.state.get("afs_gt_latents", None) is None:
            raise ValueError("AFS sequence is not initialized. Please call setup_afs_sequence first.")
        ...
        if len(denoising_steps) <= 0:
            raise ValueError("Empty denoising_step_list for AFS debug rollout.")
        teacher_future_gt_num_chunks = max(0, int(teacher_future_gt_num_chunks))

        student_chunks = list(self.state.get("afs_student_chunks", []))
        teacher_chunks = list(self.state.get("afs_teacher_chunks", []))
        ...
        while current_length < window_frames:
            ...
            if student_chunk is None or teacher_chunk is None:
                raise RuntimeError("AFS debug rollout produced no outputs.")
            ...
            generated_spans = self.state["afs_generated_chunk_spans"]
            generated_spans.append((int(abs_chunk_start), int(current_num_frames)))
            if self.afs_teacher_context_mode == "gt_kv" and len(generated_spans) >= 2:
                replace_start, replace_num_frames = generated_spans[-2]
                replace_end = replace_start + replace_num_frames
                if gt_latents.shape[1] >= replace_end:
                    gt_chunk = gt_latents[:, replace_start:replace_end]
        ...
        self.state["current_length"] = current_length
        self.state["afs_student_chunks"] = student_chunks
        self.state["afs_teacher_chunks"] = teacher_chunks
